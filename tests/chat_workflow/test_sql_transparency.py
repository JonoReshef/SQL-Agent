...
def test_execute_query_tracks_multiple_queries(mock_run_query):
    ...
    assert result["executed_queries"][0].query == "SELECT COUNT(*) FROM emails_processed"
    assert result["executed_queries"][1].query == "SELECT sender FROM emails_processed LIMIT 10"
...
def test_chat_state_has_executed_queries_field():
    ...
    state = ChatState(messages=[HumanMessage(content="test")], executed_queries=[query_exec])
...
def test_executed_queries_add_reducer():
    ...
    query1 = QueryExecution(query="SELECT 1", explanation="First query", result_summary="Result 1")
    state = ChatState(messages=[], executed_queries=[query1])

    # Simulate adding more queries (mimicking LangGraph's add reducer behavior)
    query2 = QueryExecution(query="SELECT 2", explanation="Second query", result_summary="Result 2")
    query3 = QueryExecution(query="SELECT 3", explanation="Third query", result_summary="Result 3")
