...
from agent.chat_workflow.prompts import DATABASE_SCHEMA_PROMPT, WESTBRAND_SYSTEM_PROMPT
from agent.chat_workflow.utils.db_wrapper import BLACK_LIST, get_sql_database
from agent.chat_workflow.utils.tools import get_schema_tool, run_query_tool
    ...
    try:
        ...
        if (
            ...
            tool_call_names = list(
                set([tool.get("name", "") for tool in response.tool_calls])
            )  # type: ignore
            ...
            if "run_query_tool" in tool_call_names:
                total_queries = len(
                    [
                        ...
                        if tool.get("name") == "run_query_tool"
                    ]
                )
                tool_call_message += f" Running {total_queries} {'query' if total_queries == 1 else 'queries'}. "
