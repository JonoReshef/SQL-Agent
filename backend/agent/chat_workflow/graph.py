...
from agent.chat_workflow.nodes.enrich_question import enrich_question_node
from agent.chat_workflow.nodes.execute_query import execute_query_node
from agent.chat_workflow.nodes.generate_explanations import generate_explanations_node
from agent.chat_workflow.nodes.generate_query import generate_query_node
...
def should_continue(
    state: ChatState,
) -> Literal["execute_query", "generate_explanations"]:
    """
...
def create_chat_graph() -> CompiledStateGraph:
    ...
    )  # Use custom node for query tracking
    workflow.add_node(
