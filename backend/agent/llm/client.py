...
def get_llm_client(
    ...
    # If a structured output format is provided, wrap the LLM to enforce it
    if output_structure:
...
llm_gpt51 = AzureChatOpenAI(
    api_key=os.getenv("AZURE_LLM_API_KEY"),  # type: ignore
    azure_endpoint=os.getenv("AZURE_LLM_ENDPOINT"),  # type: ignore
    azure_deployment="gpt-5.1",  # type: ignore
    api_version="",  # type: ignore
    verbose=False,
    reasoning_effort="low",
...
llm_gpt41_mini = AzureChatOpenAI(
    api_key=os.getenv("AZURE_LLM_API_KEY"),  # type: ignore
    azure_endpoint=os.getenv("AZURE_LLM_ENDPOINT"),  # type: ignore
    azure_deployment="gpt-4.1-mini",  # type: ignore
    api_version="2024-08-01-preview",  # type: ignore
