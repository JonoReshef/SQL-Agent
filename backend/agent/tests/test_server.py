...
from agent.models.chat_models import QueryExecution, QueryExplanation
from agent.server.server import app
...
def test_chat_endpoint_success(mock_get_graph, client):
    ...
    response = client.post(
        "/chat",
        json={"message": "How many emails are there?", "thread_id": "test-thread-123"},
    )
    ...
    assert (
        data["executed_queries"][0]["query"] == "SELECT COUNT(*) FROM emails_processed;"
    )
...
def test_chat_endpoint_no_queries(mock_get_graph, client):
    ...
    # Make request
    response = client.post(
        "/chat", json={"message": "Hello", "thread_id": "test-thread-456"}
    )
...
def test_chat_endpoint_no_response(mock_get_graph, client):
    ...
    # Make request
    response = client.post(
        "/chat", json={"message": "Test", "thread_id": "test-thread-789"}
    )
...
def test_chat_endpoint_exception(mock_get_graph, client):
    ...
    # Make request
    response = client.post(
        "/chat", json={"message": "Test", "thread_id": "test-thread-error"}
    )
...
def test_chat_endpoint_query_without_explanation(mock_get_graph, client):
    ...
    response = client.post(
        "/chat",
        json={"message": "Run query", "thread_id": "test-thread-no-explanation"},
    )
