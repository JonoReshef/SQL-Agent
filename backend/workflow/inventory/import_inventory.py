...
from workflow.inventory.parser import parse_inventory_batch
...
def import_inventory(
    ...
    gaskets = [
        ...
        if "gasket" in item.get("raw_description", "").lower()
    ]
    nut_items = [
        item for item in raw_items if "nut" in item.get("raw_description", "").lower()
    ]
    washers = [
        item
        for item in raw_items
        if "washer" in item.get("raw_description", "").lower()
    ]

    selected_items = [
        item for lists in zip(*[gaskets, nut_items, washers]) for item in lists
    ]
